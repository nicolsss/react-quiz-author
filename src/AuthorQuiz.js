 ...
 const Turn = ({author,books,highlight,onAnswerSelected})=>{
    ...
    const highlightToBgColor = (highlight)=>{
     ...
     return(
        ...
        <div className="col-6">
            {books.map((title)=> <div key={title}>{<Book title={title} key={title} onClick={onAnswerSelected}/>}</div>)}
        </div>
   </div>
     );
 }
     ...
     return(
         <div id="footer" className="row">
             <div className="col-12">
                 <p className="text-muted credit">All images are from <a href="htttp://wikimedia.org/wiki/M">Wikimedia Commons</a></p>
             </div>
         </div>
     ) 
 }
